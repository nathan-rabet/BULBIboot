...
int
devopen(char *device) {
    ttyfd = open(device,O_RDWR);	/* Open device */
    return (ttyfd >= 0);		/* Return 1 if success, 0 if failure */
}
...
devrestore(void) {
    ...
    pktmode(OFF);			/* Put device in normal mode */
    return(1);
}
...
int
devclose(void) {
    ttyfd = close(ttyfd);		/* Close device */
    return(1);
}

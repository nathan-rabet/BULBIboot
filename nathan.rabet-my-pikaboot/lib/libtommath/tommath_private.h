...
#    include <stdlib.h>
#    define MP_MALLOC(size) kmalloc(size)
...
#ifdef _MSC_VER
/* Prevent false positive: not enough arguments for function-like macro
 * invocation */
...
/* Static assertion */
#define MP_STATIC_ASSERT(msg, cond)                                            \
    typedef char mp_static_assert_##msg[(cond) ? 1 : -1];

#define MP_SIZEOF_BITS(type) ((size_t)CHAR_BIT * sizeof(type))

#define MP_MAX_COMBA                                                           \
    (int)(1uL << (MP_SIZEOF_BITS(mp_word) - (2u * (size_t)MP_DIGIT_BIT)))
#define MP_WARRAY                                                              \
    (int)(1uL << ((MP_SIZEOF_BITS(mp_word) - (2u * (size_t)MP_DIGIT_BIT)) + 1u))
...
/* Minimum number of available digits in mp_int, MP_DEFAULT_DIGIT_COUNT >=
 * MP_MIN_DIGIT_COUNT
 ...
 */
#define MP_MIN_DIGIT_COUNT                                                     \
    MP_MAX(3,                                                                  \
           (((int)MP_SIZEOF_BITS(uint64_t) + MP_DIGIT_BIT) - 1)                \
               / MP_DIGIT_BIT)
MP_STATIC_ASSERT(prec_geq_min_prec,
                 MP_DEFAULT_DIGIT_COUNT >= MP_MIN_DIGIT_COUNT)
...
#if defined(__STDC_IEC_559__) || defined(__GCC_IEC_559) || defined(__x86_64__) \
    || defined(_M_X64) || defined(_M_AMD64) || defined(__i386__)               \
    || defined(_M_X86) || defined(_M_IX86) || defined(__aarch64__)             \
    || defined(__arm__)
...
MP_PRIVATE mp_err s_mp_div_recursive(const mp_int *a, const mp_int *b,
                                     mp_int *q, mp_int *r) MP_WUR;
MP_PRIVATE mp_err s_mp_div_school(const mp_int *a, const mp_int *b, mp_int *c,
                                  mp_int *d) MP_WUR;
MP_PRIVATE mp_err s_mp_div_small(const mp_int *a, const mp_int *b, mp_int *c,
                                 mp_int *d) MP_WUR;
MP_PRIVATE mp_err s_mp_exptmod(const mp_int *G, const mp_int *X,
                               const mp_int *P, mp_int *Y, int redmode) MP_WUR;
MP_PRIVATE mp_err s_mp_exptmod_fast(const mp_int *G, const mp_int *X,
                                    const mp_int *P, mp_int *Y,
                                    int redmode) MP_WUR;
MP_PRIVATE mp_err s_mp_invmod(const mp_int *a, const mp_int *b,
                              mp_int *c) MP_WUR;
MP_PRIVATE mp_err s_mp_invmod_odd(const mp_int *a, const mp_int *b,
                                  mp_int *c) MP_WUR;
MP_PRIVATE mp_err s_mp_log(const mp_int *a, mp_digit base, int *c) MP_WUR;
MP_PRIVATE mp_err s_mp_montgomery_reduce_comba(mp_int *x, const mp_int *n,
                                               mp_digit rho) MP_WUR;
MP_PRIVATE mp_err s_mp_mul(const mp_int *a, const mp_int *b, mp_int *c,
                           int digs) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_balance(const mp_int *a, const mp_int *b,
                                   mp_int *c) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_comba(const mp_int *a, const mp_int *b, mp_int *c,
                                 int digs) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_high(const mp_int *a, const mp_int *b, mp_int *c,
                                int digs) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_high_comba(const mp_int *a, const mp_int *b,
                                      mp_int *c, int digs) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_karatsuba(const mp_int *a, const mp_int *b,
                                     mp_int *c) MP_WUR;
MP_PRIVATE mp_err s_mp_mul_toom(const mp_int *a, const mp_int *b,
                                mp_int *c) MP_WUR;
...
MP_PRIVATE mp_err s_mp_radix_size_overestimate(const mp_int *a, const int radix,
                                               size_t *size);
...
#define MP_GET_MAG(name, type)                                                 \
    ...
    {                                                                          \
        int i = MP_MIN(                                                        \
            a->used,                                                           \
            (int)((MP_SIZEOF_BITS(type) + MP_DIGIT_BIT - 1) / MP_DIGIT_BIT));  \
        type res = 0u;                                                         \
        while (i-- > 0)                                                        \
        {                                                                      \
            res <<=                                                            \
                ((MP_SIZEOF_BITS(type) <= MP_DIGIT_BIT) ? 0 : MP_DIGIT_BIT);   \
            res |= (type)a->dp[i];                                             \
            if (MP_SIZEOF_BITS(type) <= MP_DIGIT_BIT)                          \
            {                                                                  \
                break;                                                         \
